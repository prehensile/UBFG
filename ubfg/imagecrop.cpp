...
void ImagePacker::crop(QList<packedImage*> *images)
{
    int w, h, x, y;
    bool t;
    if(trim) for(int i = 0; i < images->size(); i++)
    {
        const QRgb &pix = images->at(i)->img.pixel(0,0);
        t = true;
        //top trimming
        for(y = 0; y < images->at(i)->img.height(); y++)
        {
            for(int j = 0; j < images->at(i)->img.width(); j++)
        ...
        for(x = 0; x < images->at(i)->img.width(); x++){
            for(int j = y; j < images->at(i)->img.height(); j++)
        ...
        if(w > 0)
            newImg = images->at(i)->img.copy(QRect(x-borderLeft, y-borderTop, w+borderLeft+borderRight, h+borderTop+borderBottom));
